...
export default function PostPurchaseProcess() {
  return (
    <section id="process" className="section-y scroll-mt-16" aria-labelledby="process-heading">
      <div className="container-x">
          ...
          {postPurchaseProcess.steps.map((step, i) => (
            <Reveal key={step.step} delay={i * 0.07} className="h-full">
              <article className="card card-hover h-full p-6 flex flex-col border-charcoal bg-ash/40">
                <span className="inline-flex size-9 items-center justify-center rounded-full bg-phosphor-green/10 border border-phosphor-green/30 text-xs font-mono font-bold text-phosphor-green">
                  {step.step}
                </span>
                <h3 className="mt-4 text-sm sm:text-base font-semibold text-snow">
                  {step.title}
                </h3>
        ...
        <Reveal delay={0.3} className="mt-10 text-center">
          <div className="inline-flex flex-wrap items-center justify-center gap-2 text-xs text-silver-mist">
            ...
            <a href="/setup" className="font-semibold text-phosphor-green hover:underline inline-flex items-center gap-1">
